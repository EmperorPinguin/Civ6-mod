# Contributor Docs

This folder collects the contributor notes that should stay easier to scan than one large README.

## Pages

- [Repository layout](repository-layout.md) - where the main ruleset, art, scripts, and metadata live.
- [Contributing](contributing.md) - general contributor workflow and expectations.
- [Validation](validation.md) - checks to run before opening a PR.
- [Policy card obsolescence](policy-card-obsolescence.md) - how obsolete policy cards should be gated, removed, and documented.
- [AI contributor guidance](ai-contributors.md) - extra conventions for AI-assisted edits.

## Quick Workflow

1. Keep each change narrowly scoped.
2. Match original Civilization VI mechanics unless a difference is intentional.
3. Run the relevant validation before pushing.
4. Update docs when changing contributor-facing behavior.
